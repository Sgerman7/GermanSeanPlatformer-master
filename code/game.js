...
// Plasma is initialized based on the character, but otherwise has a
// size and position
function Plasma(pos, ch) {
  this.pos = pos;
  ...
  if (ch == "=") {
    ...
    // Drip Plasma. Repeat back to this pos.
    this.speed = new Vector(0, 3);
    this.repeatPos = pos;
  }
}
Plasma.prototype.type = "plasma";
...
Plasma.prototype.act = function(step, level) {
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  // Handle Plasma by calling playerTouched
  if (obstacle)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // if the player touches Plasma and the player hasn't won
  // Player loses
  if (type == "plasma" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
